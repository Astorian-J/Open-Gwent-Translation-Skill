...
- **Agent-agnostic** — Every script has a `--json` flag with a unified envelope `{success, exit_code, data, errors}`. Works with Claude Code, OpenClaw, Hermes, or any agent. Python 3.10+ stdlib only, zero dependencies, and runs on macOS, Linux, and Windows.
...
## Three Tiers: Pro / Lite / Flash

The skill ships as three tiers, all installed together by `install.sh`.
...
| Content | Skill |
|---------|-------|
| Long articles (meta reports, BC proposals, card analysis) | `gwent-translation-pro` (full pipeline) |
| Chat messages, comments, single sentences | `gwent-translation-lite` (chat-length gate) |
| Live chat at full speed — single pass, in-turn self-check, no machine gate | `gwent-translation-flash` (table first; hard triggers force full-corpus lookup) |
...
│   ├── ambiguous_names.md       # Disambiguation (63 groups, machine-verified against the card db)
│   ├── version_map.md           # Version-specific terms
│   ├── phase_c_checklist.md     # Self-check rules
│   ├── translation_workflow.md  # Workflow reference
│   ├── term_decisions.md        # Terminology ruling log (reference changes need a ruling entry first)
...
│   ├── build_card_names_reference.py  # Build card_names_4lang.json (--check: diff mode for patches)
...
└── flash/                   # Flash skill — live-chat speed (1 pass, in-turn self-check)
    └── SKILL.md                  # Flash skill workflow
```
...
Install to `~/.claude/skills/gwent-translation-pro/` and restart Claude Code. The `install.sh` script installs **all three tiers together**: the main skill (`gwent-translation-pro`), `gwent-translation-lite`, and `gwent-translation-flash`. Triggers: `/gwent-translation-pro`, "translate Gwent article", "Gwent translation"; for lite — "翻一下这句" / chat translation triggers; for flash — "昆特秒翻" / fastest-chat triggers.
