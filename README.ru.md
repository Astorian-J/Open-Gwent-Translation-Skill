...
1. **`prepare --lite`** — сохраняет исходник и строит чатовый пакет (блокировки терминов и подсказки по неоднозначностям и сленгу; статейные разделы удалены). Таблица блокировок выводится прямо в результат команды.
2. **Перевод** — тот же тон игроков Bilibili / нейтральный английский, официальные названия карт и терминов.
3. **`finish --lite`** — сохраняет перевод и запускает шлюз: только термины / остатки / авторитет терминов, каждое нарушение содержит официальную формулировку.

Исходник без подозрительных имён собственных ("gg wp") может пропустить prepare и сразу перейти к `finish --lite`. Lite переиспользует `scripts/` и `references/` основного навыка (ноль дублирования данных), поэтому работает с Claude Code, opencode и другими агентами.
...
| Живой чат на полной скорости — без проверки результата | `gwent-translation-flash` (сначала таблица; lookup полного корпуса только для незнакомых названий) |

Все три уровня устанавливаются вместе через `install.sh`. Интерфейс lite для агентов: [`lite/AGENTS.md`](lite/AGENTS.md).
