...
Инструмент вводит заблокированную таблицу терминов, официальные эффекты карт и подсказки сленга для обеспечения точности. Полный прогон (pre → перевод → post → guard) обрабатывает примерно **30–60K токенов** в зависимости от длины статьи — примерно в 3–5 раза больше обычного перевода. Поскольку большая часть конвейера механическая (блокировка терминов, обнаружение остатков, проверка формата), он хорошо работает на **более дешёвых или бесплатных моделях** (Claude Haiku/Sonnet, GPT-4o-mini, DeepSeek и т.д.) или любом агенте с бесплатной квотой — вам не нужна самая дорогая модель.

*Количество токенов — это структурная оценка по объёму инъекции на этапе pre, а не измеренный end-to-end бенчмарк.*
