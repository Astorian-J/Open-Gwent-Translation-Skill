...
Usage:
    python scripts/translate.py prepare <source> [--date YYYY-MM] [--type general] [--direction encn|cnen] [--lite] [--json]
...
STYLE_ENC = [
    ...
    ("Dash", "No dash 「——」 anywhere: rewrite with commas, periods, or brackets — even when the source uses em-dashes"),
...
STYLE_CN = [
    ...
    ("Dash", "No em-dash (—) in the English output: use commas, periods, or parentheses instead"),
...
# Lite pack (chat-length content): style rows that matter for a short message.
# Filtered from the tables above BY LABEL so the two stay in sync — article-grade
# rows (sentence splitting, style fingerprint) would only pad what the model reads.
LITE_STYLE_ENC = ("Tone", "Voice", "Numbers", "Parentheses", "Dash", "Verbs", "Rhetoric")
LITE_STYLE_CN = ("Tone", "Numbers", "Parentheses", "Dash", "Slang", "Rhetoric")
...
def build_pack(source_path: Path, direction: str, date: str | None,
               article_type: str, pre_data: dict, lock_built: bool,
               lite: bool = False) -> str:
    """Assemble the Markdown translation pack from auto_pipeline pre's JSON output.

    lite=True builds the chat-length pack: sections finish --lite does not gate on
    (balance direction, markdown format, official effect text, Phase C checklist)
    are dropped and the style table is filtered to LITE_STYLE_* rows. The term
    lock / ambiguous / slang content — everything the lite gate enforces or that
    guides a short message — is identical to the full pack.
    """
    ...
    if lite:
        keep = LITE_STYLE_ENC if is_encn else LITE_STYLE_CN
        style_rules = [row for row in style_rules if row[0] in keep]
    ...
    if lite:
        L.append("> do not translate literally.")
    else:
    ...
    if lite:
        L.append("> - **[JUDGE 判断]** — 规则与提示：按引导用自己的判断执行（风格、俚语）。")
    else:
    ...
    # Lite skips it: chat snippets never quote effects verbatim, and the
    # injected ability text is the single largest per-card payload.
    if effects and not lite:
    ...
    # Phase C acceptance checklist — full flow only: finish --lite does not run
    # the Phase C checks, so a checklist in the lite pack would ask the agent to
    # self-check gates that will never fire.
    if not lite:
    ...
    L.append("")
    if lite:
        L.append("1. Translate the source now, following the locked terms above.")
        L.append("2. Save the translation, then gate it in ONE command (NOT final until PASS):")
        L.append("")
        L.append("   ```bash")
        L.append(
            f"   python {(SCRIPTS_DIR / 'translate.py').resolve()} finish <translated> "
            f"--source {source_path.resolve()} --direction {direction} --lite"
        )
        L.append("   ```")
        L.append("")
        L.append("> **BLOCKED** = fix per the violation list (each entry carries the official")
        L.append("> rendering) and re-run. Max 3 rounds.")
        L.append("")
    else:
...
def cmd_prepare(args: argparse.Namespace) -> None:
    ...
    pack_content = build_pack(source_path, direction, args.date, article_type,
                              pre_data, lock_built, lite=args.lite)
    try:
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    if translated_path is None:
        ...
        print("")
        lite_flag = " --lite" if args.lite else ""
        print(f"  python {self_py} finish <translated> --source {source_path.resolve()} --direction {direction}{lite_flag}")
        print("")
        print("With the translation already saved, the whole gate in one command:")
        print(f"  python {self_py} run {source_path.resolve()} --translated <translated> --direction {direction}{lite_flag}")
        sys.exit(0)
...
def main() -> None:
    ...
    prep.add_argument("--lite", action="store_true",
                      help="Chat-length pack: drops the article-grade sections finish --lite does "
                           "not gate on (balance direction, markdown format, official effect text, "
                           "Phase C checklist) and filters the style table. Term lock content is "
                           "identical. Pairs with finish --lite.")
    ...
    runp.add_argument("--lite", action="store_true",
                      help="Forwarded to prepare AND finish (chat-length pack + chat-length gate)")
