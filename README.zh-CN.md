...
本 skill 会注入锁定术语表、官方卡牌效果、黑话提示来保证准确。一次完整流程（pre → 翻译 → post → guard）大约消耗 **3-6 万 tokens**，视文章长度而定——约为裸翻译的 3-5 倍。因为流水线大部分是机械操作（术语锁定、残留检测、格式检查），用**便宜或免费模型**（Claude Haiku/Sonnet、GPT-4o-mini、DeepSeek 等）或任何带免费额度的 agent 就能跑得很好，不需要最贵的模型。

*token 数值是按 pre 阶段注入量推算的结构化估算，非端到端实测基准。*
